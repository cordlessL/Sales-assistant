...
Минимально рабочая версия (MVP).
 Telegram-бот получает текстовый запрос от покупателя. Генерирует текстовый ответ в GigaChat 1-2 товара, формирует промп для генерации картинки в ProxyApi, возвращает ответ: Картинку подписанную текстом. По необходимости задаёт уточняющий вопрос.
